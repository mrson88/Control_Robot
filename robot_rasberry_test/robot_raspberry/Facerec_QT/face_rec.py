...
def write_db(db, id, name, vector):
    ...
    db[id] = entry
    print(db)
    f = open('Facerec_QT/resources/database.db','w')
    entry = json.dumps(db)
    f.write(entry)
    f.close()
...
def read_db(db_path = 'Facerec_QT/resources/database.db'):
    try:
        f = open(db_path, 'r')
...
def clear_db(db_path = 'Facerec_QT/resources/database.db'):
...
class FaceRecognition():
    ...
    def __init__(self, threshold, image_height, image_width):
        self.fd_model = NetworkExecutor("Facerec_QT/face_rec_models/ulffd_landmark.tflite", preprocess_for_fd)
        self.fe_model = NetworkExecutor("Facerec_QT/face_rec_models/inference_model_993.tflite", preprocess_for_fe)
    ...
    def load_db(self):
        self.db = read_db("Facerec_QT/resources/database.db")
